...
static struct Papyrus_Expr
EvaluateExpr_StrCat(struct Papyrus_Expr lhs,
	struct Papyrus_Expr rhs, struct Arena* arena)
{
	if (lhs.lit.string.size == 0)
		return rhs;

	if (rhs.lit.string.size == 0)
		return lhs;

	intptr_t size = lhs.lit.string.size + rhs.lit.string.size;
	char* data = (char*)Arena_Allocate(arena, size);

	memcpy(data,
		lhs.lit.string.data, lhs.lit.string.size);

	memcpy(data + lhs.lit.string.size,
		rhs.lit.string.data, rhs.lit.string.size);

	return (struct Papyrus_Expr) {
		.kind = Papyrus_Expr_LitString,
		.ekind = 0,
		.flags = Papyrus_ExprFlags_Const,
		.type = IntrinsicType(String),
		.lit.string.data = data,
		.lit.string.size = size,
	};
}
...
Papyrus_EvaluateExpr(struct Papyrus_Expr expr, struct Arena* arena)
{
	...
	case Papyrus_Expr_Neg:
		{
			expr = Papyrus_EvaluateExpr(*expr.oper.expr, arena);

			switch (expr.type->kind)
			{
			case Papyrus_Type_Int:
				expr.lit.int_ = -expr.lit.int_;
				break;

			case Papyrus_Type_Float:
				expr.lit.float_ = -expr.lit.float_;
				break;
			}
		}
		break;
	...
	case Papyrus_Expr_Not:
		{
			...
			{
			case Papyrus_Type_Int:
				expr.lit.bool_ = expr.lit.int_ != 0;
				break;

			case Papyrus_Type_Bool:
				expr.lit.bool_ = !expr.lit.bool_;
				break;

			case Papyrus_Type_Float:
				expr.lit.bool_ = expr.lit.float_ != 0;
				break;

			case Papyrus_Type_String:
				expr.lit.bool_ = expr.lit.string.size != 0;
				break;
			}
...
#define OPERATOR(t, x, o) \
	case Papyrus_Type_ ## t: \
		expr.lit.x = lhs.lit.x o rhs.lit.x; \
		break;

#define FUNCTION(t, f) \
	case Papyrus_Type_ ## t: \
		expr = f(lhs, rhs, arena);
		break;
	...
	case Papyrus_Expr_ ## x: \
		{ \
			struct Papyrus_Expr lhs = \
				Papyrus_EvaluateExpr(*expr.oper.lhs, arena); \
			struct Papyrus_Expr rhs = \
				Papyrus_EvaluateExpr(*expr.oper.rhs, arena); \
		...
		BINARY(Add, OPERATOR(Int, int_, +) OPERATOR(Float, float_, +)
			FUNCTION(String, EvaluateExpr_StrCat));

		BINARY(Sub, OPERATOR(Int, int_, -) OPERATOR(Float, float_, -));
		BINARY(Mul, OPERATOR(Int, int_, *) OPERATOR(Float, float_, *));
		BINARY(Div, OPERATOR(Int, int_, /) OPERATOR(Float, float_, /));

		BINARY(Mod, OPERATOR(Int, int_, %));
#undef OPERATOR
#undef FUNCTION
#undef BINARY
